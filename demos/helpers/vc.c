...
//
// static Olive_Canvas vc_render(float dt)
// {
...
int main(void) {
...

	{
		...
		for (;;) {
			...
			if (!pause) {
				// Render the texture
				Olive_Canvas oc_src = vc_render(dt);
				...
				for (size_t y = 0; y < vc_sdl_actual_height; ++y) {
					// TODO: it would be cool if Olive_Canvas supported pitch in bytes
...
static uint32_t vc_term_compress_pixels_chunk(Olive_Canvas oc) {
	size_t r = 0;
	size_t g = 0;
	size_t b = 0;
	size_t a = 0;
	...
	for (size_t y = 0; y < oc.height; ++y) {
		for (size_t x = 0; x < oc.width; ++x) {
			r += OLIVE_RED(OLIVE_PIXEL(oc, x, y));
			g += OLIVE_GREEN(OLIVE_PIXEL(oc, x, y));
			b += OLIVE_BLUE(OLIVE_PIXEL(oc, x, y));
			a += OLIVE_ALPHA(OLIVE_PIXEL(oc, x, y));
		}
	}
...
static void vc_term_compress_pixels(Olive_Canvas oc) {
	...
	for (size_t y = 0; y < vc_term_scaled_down_height; ++y) {
		for (size_t x = 0; x < vc_term_scaled_down_width; ++x) {
			Olive_Canvas soc = olive_subcanvas(
			...
			int r = OLIVE_RED(cp);
			int g = OLIVE_GREEN(cp);
			int b = OLIVE_BLUE(cp);
			int a = OLIVE_ALPHA(cp);
			r = a * r / 255;
			g = a * g / 255;
			b = a * b / 255;
			int h, s, l;
