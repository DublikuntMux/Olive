...
Olive_Canvas vc_render(float dt) {
	...
	for (size_t i = 0; i < faces_count; ++i) {
		...
		int lx, hx, ly, hy;
		if (olive_normalize_triangle(oc.width, oc.height, p1[0], p1[1], p2[0],
					p2[1], p3[0], p3[1], &lx, &hx, &ly, &hy)) {
			for (int y = ly; y <= hy; ++y) {
				for (int x = lx; x <= hx; ++x) {
					int u1, u2, det;
					if (olive_barycentric(p1[0], p1[1], p2[0], p2[1], p3[0], p3[1], x, y,
								&u1, &u2, &det)) {
						int u3 = det - u1 - u2;
						...
						if (1.0f / far < z && z < 1.0f / near &&
							...
							zbuffer[y * WIDTH + x] = z;
							OLIVE_PIXEL(oc, x, y) =
									olive_mix_colors3(0xFF1818FF, 0xFF18FF18, 0xFFFF1818, u1, u2, det);
