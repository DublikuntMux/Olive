...
static bool vc_sdl_resize_texture(SDL_Renderer *renderer, size_t new_width,
                                  size_t new_height) {
  if (vc_sdl_texture != NULL)
    SDL_DestroyTexture(vc_sdl_texture);
  vc_sdl_actual_width = new_width;
  vc_sdl_actual_height = new_height;
  vc_sdl_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
...

  {
    ...
    for (;;) {
      ...
      if (!pause) {
        ...
        if (oc_src.width != vc_sdl_actual_width ||
            oc_src.height != vc_sdl_actual_height) {
        ...
        void *pixels_dst;
        int pitch;
        if (SDL_LockTexture(vc_sdl_texture, &window_rect, &pixels_dst, &pitch) <
            0)
          return_defer(1);
        for (size_t y = 0; y < vc_sdl_actual_height; ++y) {
          // TODO: it would be cool if Olivec_Canvas supported pitch in bytes
          // instead of pixels It would be more flexible and we could draw on
          // the locked texture memory directly
          memcpy((char *)pixels_dst + y * pitch,
                 oc_src.pixels + y * vc_sdl_actual_width,
      ...
      if (SDL_RenderClear(renderer) < 0)
        return_defer(1);
      if (SDL_RenderCopy(renderer, vc_sdl_texture, &window_rect, &window_rect) <
          0)
        return_defer(1);
...
int hsl256[][3] = {
    ...
    {197, 100, 26}, {207, 100, 34}, {213, 100, 42}, {217, 100, 50},
    {120, 100, 26}, {162, 100, 26}, {180, 100, 26}, {193, 100, 34},
    {202, 100, 42}, {208, 100, 50}, {120, 100, 34}, {152, 100, 34},
    {166, 100, 34}, {180, 100, 34}, {191, 100, 42}, {198, 100, 50},
    {120, 100, 42}, {146, 100, 42}, {157, 100, 42}, {168, 100, 42},
    {180, 100, 42}, {189, 100, 50}, {120, 100, 50}, {142, 100, 50},
    ...
    {309, 100, 50}, {300, 100, 50}, {22, 100, 50},  {0, 100, 68},
    {345, 100, 68}, {330, 100, 68}, {315, 100, 68}, {300, 100, 68},
    {31, 100, 50},  {15, 100, 68},  {0, 100, 76},   {340, 100, 76},
    {320, 100, 76}, {300, 100, 76}, {41, 100, 50},  {30, 100, 68},
    {20, 100, 76},  {0, 100, 84},   {330, 100, 84}, {300, 100, 84},
    {50, 100, 50},  {45, 100, 68},  {40, 100, 76},  {30, 100, 84},
...
static void vc_term_resize_char_canvas(size_t new_width, size_t new_height) {
  ...
  // Or we can resize the frame on the fly similarly to how we resize sprites in
  // olivec_sprite_*() functions.
  ...
  vc_term_scaled_down_width =
      (vc_term_actual_width * VC_TERM_ASPECT_RATIO) / VC_TERM_SCALE_DOWN_FACTOR;
  ...
  for (size_t y = 0; y < vc_term_scaled_down_height; ++y) {
    for (size_t x = 0; x < vc_term_scaled_down_width; ++x) {
      Olivec_Canvas soc = olivec_subcanvas(
          oc, x * VC_TERM_SCALE_DOWN_FACTOR / VC_TERM_ASPECT_RATIO,
          y * VC_TERM_SCALE_DOWN_FACTOR,
          VC_TERM_SCALE_DOWN_FACTOR / VC_TERM_ASPECT_RATIO,
  ...
  for (;;) {
    ...
    for (size_t y = 0; y < vc_term_scaled_down_height; ++y) {
      for (size_t x = 0; x < vc_term_scaled_down_width; ++x) {
        ...
        // using escape ANSI codes of the terminal and rendering the image
        // accordingly
        printf("\033[48;5;%dm ",
               vc_term_char_canvas[y * vc_term_scaled_down_width + x]);
      }
