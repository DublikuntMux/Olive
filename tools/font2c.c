...
void determine_bitmap_sizes(FT_Face face, size_t *width, size_t *height,
                            size_t *baseline) {
  ...
  // height of all bitmaps has to be max ascender + max descender (+1? Is
  // baseline included in asc or desc? or none?)
  ...
  for (unsigned char c = FROM_CHAR; c < TO_CHAR; ++c) {
    ...
    if (((FT_Int)face->glyph->bitmap.rows - face->glyph->bitmap_top) >
        max_descender) {
...
void write_byte_array_code_for_char_to_file(FILE *out, FT_Face face,
  ...
  for (size_t y = 0; y < height; ++y) {
    fprintf(out, "        {");
    if (y < (unsigned int)skip_lines ||
        y >= (unsigned int)(skip_lines + bitmap->rows)) {
      ...
      for (size_t x = 0; x < width; ++x) {
        ...
        fprintf(out, "%c, ",
                bitmap->buffer[bitmap->width * (y - skip_lines) + x] == 0
                    ? '0'
                    : '1');
      }
    }
...
int generate_c_file_from_ttf(const char *input_file_path,
                             const char *output_file_path, const char *name) {
  FT_Library library;
  FT_Face face;
  ...
  if (error) {
    fprintf(stderr,
            "ERROR: there was a problem initializing the FreeType library\n");
    return 1;
  }
  ...
  error = FT_Set_Char_Size(face, 0, 16 * 64, 16, 16);
  ...
  for (unsigned char c = FROM_CHAR; c < TO_CHAR; ++c) {
    write_byte_array_code_for_char_to_file(out, face, c, width, height,
                                           baseline);
  }
  ...
  if (name == NULL) {
    ...
    for (size_t i = 0; i < n; ++i) {
      if (!isalnum(name[i]) && name[i] != '_') {
        fprintf(stderr, "ERROR: name can only contains alphanumeric characters "
                        "and underscores\n");
        return 1;
      }
    }
