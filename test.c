...
Olivec_Canvas test_alpha_blending(void) {
  ...
  olivec_triangle(oc, 0, height - 1, width - 1, height - 1, width / 2, 0,
                  0xBB20AAAA);
  olivec_triangle3c(oc, 0, 0, width - 1, 0, width / 2, height - 1, 0xBB2020AA,
...
Olivec_Canvas test_hello_world_text_rendering(void) {
  ...
  olivec_fill(oc, BACKGROUND_COLOR);
  olivec_text(oc, text,
              oc.width / 2 - OLIVEC_DEFAULT_FONT_WIDTH * size * text_len / 2,
              oc.height / 2 - OLIVEC_DEFAULT_FONT_HEIGHT * size / 2,
              olivec_default_font, size, FOREGROUND_COLOR);
  return oc;
}
...
Olivec_Canvas test_sprite_blend_flip_cut(void) {
  ...
  olivec_fill(dst, RED_COLOR);
  olivec_sprite_blend(dst, -width / 2, -height / 2, width, height, src);
  olivec_sprite_blend(dst, width - 1 + width / 2, -height / 2, -width, height,
                      src);
  olivec_sprite_blend(dst, -width / 2, height - 1 + height / 2, width, -height,
                      src);
  olivec_sprite_blend(dst, width - 1 + width / 2, height - 1 + height / 2,
                      -width, -height, src);
  return dst;
}
...
Olivec_Canvas test_sprite_blend_vs_copy(void) {
  Olivec_Canvas tsodinCup = olivec_canvas(tsodinCup_pixels, tsodinCup_width,
  ...
  olivec_sprite_copy(dst, 0, tsodinCup.height, tsodinCup.width,
...
Olivec_Canvas test_barycentric_overflow(void) {
  ...
  olivec_triangle3c(dst, w / 4, h / 4, w / 2, 0, 0, h, 0xFF0000FF, 0xFF00FF00,
                    0xFFFF0000);
  return dst;
}
...
Olivec_Canvas test_triangle_order_flip(void) {
  ...
  Olivec_Canvas dst = canvas_alloc(w, h);
  olivec_fill(dst, 0xFF181818);
  olivec_triangle3c(dst, w / 4, h / 4, 0, h, w, 0, 0xFF00FF00, 0xFFFF0000,
                    0xFF0000FF);
  return dst;
}
...
Olivec_Canvas test_fill_ellipse(void) {
  ...
  olivec_fill(dst, BACKGROUND_COLOR);
  olivec_ellipse(dst, dst.width / 2, dst.height / 2, dst.width / 3,
