...
Olive_Canvas test_fill_rect(void) {
	...
	olive_rect(oc, width / 2 - width / 8, height / 2 - height / 8, width / 4,
			height / 4, RED_COLOR);
	olive_rect(oc, width - 1, height - 1, -width / 2, -height / 2, GREEN_COLOR);
	olive_rect(oc, -width / 4, -height / 4, width / 2, height / 2, BLUE_COLOR);
	return oc;
}
...
Olive_Canvas test_fill_circle(void) {
	...
	Olive_Canvas oc = canvas_alloc(width, height);
	olive_fill(oc, BACKGROUND_COLOR);
	olive_circle(oc, 0, 0, width / 2, RED_COLOR);
	olive_circle(oc, width / 2, height / 2, width / 4, BLUE_COLOR);
	olive_circle(oc, width * 3 / 4, height * 3 / 4, -width / 4, GREEN_COLOR);
	return oc;
}
...
Olive_Canvas test_alpha_blending(void) {
	...
	Olive_Canvas oc = canvas_alloc(width, height);
	olive_fill(oc, BACKGROUND_COLOR);
	olive_rect(oc, 0, 0, width * 3 / 4, height * 3 / 4, RED_COLOR);
	olive_rect(oc, width - 1, height - 1, -width * 3 / 4, -height * 3 / 4,
			0x5520AA20);
	olive_circle(oc, width / 2, height / 2, width / 4, 0xBBAA2020);
...
Olive_Canvas test_checker_example(void) {
	...
	for (int y = 0; y < rows; ++y) {
		for (int x = 0; x < cols; ++x) {
			...
			olive_rect(oc, x * cell_width, y * cell_height, cell_width, cell_height,
					color);
		}
	}
...
Olive_Canvas test_circle_example(void) {
	...
	for (int y = 0; y < rows; ++y) {
		for (int x = 0; x < cols; ++x) {
			...
			olive_circle(oc, x * cell_width + cell_width / 2,
...
Olive_Canvas test_frame(void) {
...

	{
		size_t w = width / 2;
		size_t h = height / 2;
		olive_frame(oc, 0, 0, w, h, 1, RED_COLOR);
	}

	{ olive_frame(oc, width / 2, height / 2, width, height, 1, GREEN_COLOR); }
	...
	// Odd thiccness
	{
		...
		size_t h = height / 2;
		size_t t = 5;
		olive_frame(oc, width / 2 - w / 2, height / 2 - h / 2, w, h, t,
				WHITE_COLOR);
		olive_frame(oc, width / 2 - w / 2, height / 2 - h / 2, w, h, 1, RED_COLOR);
	}
...
Olive_Canvas test_empty_rect(void) {
	...
	olive_rect(dst, w / 2, h / 2, 0, 0, FOREGROUND_COLOR);
	return dst;
}
